...
file = "good"
i="0"
passwrd = "tadgyoimetviusts"
addr = "varunram2748@gmail.com"
item =""
subject = ""
body = ""
...
def pwd_speak():
    ...
    while not end:
        texttospeech("What is the type of character? number or a letter", file + i)
        i = i + str(1)
        char_type = speechtotext(3).lower()
        if char_type == 'number':
            while True:
                texttospeech("say the number", file + i)
                i = i + str(1)
                char  = speechtotext(3)
                print("number: ", char)
                texttospeech("is this what you meant " + char + " say continue to confirm or break to enter again", file + i)
                i = i + str(1)
                yes_no = speechtotext(3).lower()
                if yes_no == 'continue':
                    pwd = pwd + char
                    break
                if yes_no == 'break':
                    texttospeech("could not understand what you meant, please enter again", file + i)
                    i = i + str(1)
                    continue
                
        elif char_type == 'letter':
            texttospeech("is it a capital letter or a small letter", file + i)
            i = i + str(1)
            c_or_l = speechtotext(3).lower()
            while True:
                texttospeech("say the character", file + i)
                i = i + str(1)
                char = speechtotext(3).lower()
        ...
        if len(pwd) >= 6:
            texttospeech("is this is the end password? say continue or break.", file + i)
            i = i + str(1)    
            yes_no = speechtotext(3).lower()
            if yes_no == 'continue':
                end = True
        ...
        texttospeech("is there anymore characters you want to say", file + i)
        i = i + str(1)
        yes_no = speechtotext(3).lower()
        if yes_no == 'continue':
            continue
        
        texttospeech("could not understand what you meant, please say again", file + i)
        i = i + str(1)
        speechtotext(3).lower() 
    return pwd
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        while (flag):
            ...
            i = i + str(1)
            addr = speechtotext(8)
            if "at the rate" in addr:
                addr = addr.replace("at the rate", "@")
            if "dot" in addr:
                addr = addr.replace(" dot ", ".")
            
            addr = addr.replace(" ", "").lower()
            print("Actual text:",addr)
            addr = "varunram2748@gmail.com"
            print(addr)
            
            if addr != 'N':
                texttospeech("You meant " + addr + " say continue to confirm or no to enter again", file + i)
                i = i + str(1)
                say = speechtotext(3).lower()
                if say == 'continue':
                    flag = False
            else:
        ...
        while (flag):
            ...
            # passwrd = pwd_speak()
            passwrd="jkylfzzaymghabls"
            print("default pass",passwrd)
            
            if addr != 'N':
                texttospeech("You meant " + passwrd + " say continue to confirm or break to enter again", file + i)
                i = i + str(1)
                say = speechtotext(3).lower()
                if say == 'continue':
                    flag = False
...
def compose_view(request):
    ...
    if request.method == 'POST':
        ...
        while flag1:
            while flag:
                texttospeech("enter receiver's name:", file + i)
                i = i + str(1)
                to = ""
                to = speechtotext(15)
                emails_clients = {"varun":"ramreddy0711@gmail.com", "sujit": "sujithkeesara@gmail.com"}
                if to != 'N':
                    recv_mail = emails_clients.get(to.replace(" ", "").lower(), None)
                    if recv_mail is None:
                        texttospeech("Email not found in the database", file + i)
                        texttospeech("Please enter the name of the receiver", file + i)
                        continue
                    print("to:",to)
                    texttospeech(f"Mail address for {to} is {recv_mail}", file + i)
                    texttospeech("You meant " + to + " say continue to confirm or break to enter again", file + i)
                    to = recv_mail
                    i = i + str(1)
                    say = str(speechtotext(5)).lower()
                    if say == 'continue':
                        toaddr.append(to)
                        flag = False
                else:
                    texttospeech("could not understand what you meant", file + i)
                    i = i + str(1)
            texttospeech("Do you want to enter more recipients ?  Say continue or break.", file + i)
            i = i + str(1)
            say1 = str(speechtotext(3)).lower()
            if say1 == 'break':
                flag1 = False
            flag = True
        ...
        texttospeech("any attachment? say continue or break", file + i)
        i = i + str(1)
        x = str(speechtotext(3)).lower()
        x = x.lower()
        if x == 'continue':
            texttospeech("Do you want to record an audio and send as an attachment?", file + i)
            i = i + str(1)
            say = str(speechtotext(2)).lower()
            say = say.lower()
            if say == 'continue':
...
def reply_mail(msg_id, message):
    ...
    while(flag):
        ...
        try:
            ...
            texttospeech("Your reply could not be sent. Do you want to try again? Say continue or break.", file + i)
            i = i + str(1)
            act = str(speechtotext(3)).lower()
            act = act.lower()
            if act != 'continue':
                flag = False
...
def frwd_mail(item, message):
    ...
    while flag:
        ...
        try:
            ...
            texttospeech("Your mail could not be forwarded. Do you want to try again? Say continue or break.", file + i)
            i = i + str(1)
            act = speechtotext(3).lower()
            act = act.lower()
            if act != 'continue':
                flag = False
...
def read_mails(mail_list,folder):
    ...
    while flag :
        ...
        while flag1:
            ...
            n = speechtotext(2)
            print(n)
            texttospeech("You meant " + str(n) + ". Say continue or break.", file + i)
            i = i + str(1)
            say = speechtotext(2).lower()
            say = say.lower()
            if say == 'continue':
                flag1 = False
        n = int(n)
        ...
        if folder == 'inbox':
            texttospeech("Do you want to reply to this mail? Say continue or break. ", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
        ...
        if folder == 'inbox' or folder == 'sent':
            texttospeech("Do you want to forward this mail to anyone? Say continue or break. ", file + i)
            i = i + str(1)
            ans = speechtotext(3).lower()
            ans = ans.lower()
            print(ans)
            if ans == "continue":
        ...
        if folder == 'inbox' or folder == 'sent':
            texttospeech("Do you want to delete this mail? Say continue or break. ", file + i)
            i = i + str(1)
            ans = speechtotext(3).lower()
            ans = ans.lower()
            print(ans)
        ...
        while(flag):
            ...
            if act == 'unread':
                ...
                while True:
                    ...
                    texttospeech("You meant " + emailid + " say continue to confirm or break to enter again", file + i)
                    i = i + str(1)
                    yn = speechtotext(5)
                    yn = yn.lower()
                    if yn == 'continue':
            ...
            texttospeech("If you wish to do anything else in the inbox or logout of your mail say continue or else say break.", file + i)
            i = i + str(1)
            ans = speechtotext(3).lower()
            ans = ans.lower()
            if ans == 'continue':
    ...
    if request.method == 'POST':
        ...
        while (flag):
            ...
            if act == 'search':
                ...
                emailid = ""
                while True:
                    sent_emails = {"varun": "varunram2748@gmail.com", "sujith": "sujithkeesara@gmail.com"}
                    texttospeech("Enter name of receiver.", file + i)
                    i = i + str(1)
                    emailid = speechtotext(15)
                    emailid = sent_emails.get(emailid.replace(" ", "").lower(), None)
                    if emailid is None:
                        texttospeech("Sorry, we could not find the email ID of the person you mentioned. Please try again.", file + i)
                        continue
                    texttospeech("You meant " + emailid + " say continue to confirm or break to enter again", file + i)
                    i = i + str(1)
                    yn = speechtotext(5)
            ...
            texttospeech("If you wish to do anything else in the sent mails folder or logout of your mail say continue or else say break.", file + i)
            i = i + str(1)
            ans = speechtotext(3).lower()
            ans = ans.lower()
            if ans == 'continue':
...
def trash_view(request):
    ...
    if request.method == 'POST':
        ...
        while (flag):
            ...
            texttospeech("If you wish to do anything else in the trash folder or logout of your mail say continue or else say break.", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
